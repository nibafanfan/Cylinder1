...
def build_dataset(dataset_config,
    ...
    # print('semanticdemo', len(SemKITTI_demo))
    ...
    # print(len(demo_pt_dataset),'demoptdataset')
...
def main(args):
    ...
    #         predict_labels = predict_labels.cpu().detach().numpy()
            ...
            for count, i_demo_grid in enumerate(demo_grid):
                ...
                annotated_data = np.fromfile(outputPath, dtype=np.uint32).reshape((-1, 1))
                annotated_data = annotated_data & 0xFFFF
                unpacked = np.vectorize(learning_map.__getitem__)(annotated_data)
    #             print('aaaaaaaaaaaa')
                hist_list.append(fast_hist_crop(unpacked, demo_pt_labs[count],
                                                unique_label))
